...
const uploads =
  process.env.INTERMEDI_UPLOAD_DIR ||
  fileURLToPath(new URL("../uploads/perfis/", import.meta.url));
const fields = [
  "nomePaciente",
  "cpfPaciente",
  "telPaciente",
...
const fingerprint = (idPaciente) =>
  hash(pacientes.buscarSenhaHash(idPaciente) ?? "");
...
function publicUser(p) {
  return {
    id: Number(p.idPaciente),
    nome: p.nomePaciente,
    email: p.emailPaciente,
    cpf: p.cpfPaciente,
    telefone: p.telPaciente ?? "",
    remedioFrequente: p.medicamentoFrequentePaciente ?? "",
    fotoPerfilPaciente: p.fotoPerfilPaciente ?? null,
  };
}
...
async function body(req, limit = 32 * 1024) {
  ...
  for await (const chunk of req) {
    size += chunk.length;
    if (size > limit)
      throw fail(413, "Arquivo ou dados excedem o limite permitido.");
    chunks.push(chunk);
  }
...
async function readJson(req) {
  try {
    ...
    if (!data || typeof data !== "object" || Array.isArray(data))
      throw fail(400, "JSON inválido.");
    return data;
  } catch (error) {
    throw error.status ? error : fail(400, "JSON inválido.");
  }
}
...
function authenticated(req) {
  ...
  const session = db
    .prepare(
      "SELECT * FROM sessao_paciente WHERE token_hash = ? AND expira_em > ?",
    )
    .get(hash(token), Date.now());
  const user = session && pacientes.buscarPorId(session.id_paciente);
  if (!user || session.senha_fingerprint !== fingerprint(user.idPaciente))
    throw fail(401, "Sua sessão expirou. Faça login novamente.");
  if (req.params?.id && Number(req.params.id) !== Number(user.idPaciente))
    throw fail(403, "Acesso não permitido.");
  return user;
}
...
function validate(data, partial = false) {
  ...
  for (const name of [
    ...
    if ((!partial || data[name] !== undefined) && !data[name]?.trim())
      throw fail(400, "Preencha nome, CPF, e-mail e senha.");
  }
  if (
    data.emailPaciente !== undefined &&
    !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.emailPaciente)
  )
    throw fail(400, "E-mail inválido.");
}

function duplicate(data, id = -1) {
  if (pacientes.existeDuplicado(data.emailPaciente, data.cpfPaciente, id))
    throw fail(409, "E-mail ou CPF já cadastrado.");
}
...
export default function appRoutes(router) {
  ...
  route("post", "/api/auth/register", async (req, res) => {
    ...
    json(res, 201, {
      ...
      user: publicUser(pacientes.buscarPorId(created.idPaciente)),
    });
  });

  route("post", "/api/auth/login", async (req, res) => {
    const { email, senha } = await readJson(req);
    if (typeof email !== "string" || typeof senha !== "string" || !senha)
      throw fail(400, "Informe e-mail e senha.");
    ...
    db.prepare("DELETE FROM sessao_paciente WHERE expira_em <= ?").run(
      Date.now(),
    );
    db.prepare(
      "INSERT INTO sessao_paciente (token_hash, id_paciente, senha_fingerprint, expira_em) VALUES (?, ?, ?, ?)",
    ).run(
      hash(token),
      user.idPaciente,
      fingerprint(user.idPaciente),
      Date.now() + 7 * 86400000,
    );
  ...
  route("post", "/api/auth/logout", async (req, res) => {
    ...
    db.prepare("DELETE FROM sessao_paciente WHERE token_hash = ?").run(
      hash(token),
    );
    json(res, 200, { message: "Sessão encerrada." });
  });

  route("get", "/api/pacientes/me", async (req, res) =>
    json(res, 200, { user: publicUser(authenticated(req)) }),
  );
  route("put", "/api/pacientes/me", async (req, res) => {
    ...
    const merged = Object.fromEntries(
      fields.map((name) => [name, data[name] ?? user[name] ?? ""]),
    );
    ...
    emTransacao(() => {
      ...
      if (data.senhaPaciente !== undefined) {
        ...
        db.prepare(
          "DELETE FROM sessao_paciente WHERE id_paciente = ? AND token_hash != ?",
        ).run(user.idPaciente, current);
        db.prepare(
          "UPDATE sessao_paciente SET senha_fingerprint = ? WHERE token_hash = ?",
        ).run(fingerprint(user.idPaciente), current);
      }
    });
    json(res, 200, {
      user: publicUser(pacientes.buscarPorId(user.idPaciente)),
    });
  });
  ...
  route("put", "/api/pacientes/:id/foto", async (req, res) => {
    ...
    try {
      ...
      throw fail(400, "Envie a foto como multipart/form-data.");
    }
    const file = form.get("foto");
    if (!file || typeof file === "string" || !file.size)
      throw fail(400, "Selecione uma foto.");
    if (file.size > 5 * 1024 * 1024)
      throw fail(413, "A foto deve ter no máximo 5 MB.");
    const buffer = Buffer.from(await file.arrayBuffer());
    const ext = buffer.subarray(0, 3).equals(Buffer.from([255, 216, 255]))
      ? "jpg"
      : buffer
            .subarray(0, 8)
    ...
    try {
      pacientes.atualizarFoto(user.idPaciente, url);
    } catch (error) {
  ...
  route("get", "/uploads/perfis/:file", async (req, res) => {
    ...
    if (!/^[a-f0-9]{48}\.(jpg|png|webp)$/.test(name))
      throw fail(404, "Foto não encontrada.");
    let bytes;
    try {
      bytes = await readFile(path.join(uploads, name));
    } catch (error) {
      if (error.code === "ENOENT") throw fail(404, "Foto não encontrada.");
      throw error;
    }
...
async function removePhoto(url) {
  const name = url?.match(
    /^\/uploads\/perfis\/([a-f0-9]{48}\.(?:jpg|png|webp))$/,
  )?.[1];
